  ...
  override def iterator(): Iterator[(K, V)] = {
    ...
    new NextIterator[(K, V)] {
      override protected def getNext(): (K, V) = {
        unsafeRowPairIterator.dropWhile { rowPair =>
          stateTypesEncoder.isExpired(rowPair.value, batchTimestampMs)
        }
        if (unsafeRowPairIterator.hasNext) {
          val currentRowPair = unsafeRowPairIterator.next()
          if (!stateTypesEncoder.isExpired(currentRowPair.value, batchTimestampMs)) {
